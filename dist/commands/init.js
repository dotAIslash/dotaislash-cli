...
export async function init(targetDir = process.cwd(), options = {}) {
    ...
    // Create context.json
    const context = options.minimal
        ? createMinimalContext()
        : createDefaultContext();
    writeFileSync(join(aiPath, 'context.json'), JSON.stringify(context, null, 2) + '\n');
    ...
    if (options.profile) {
        ...
        writeFileSync(join(aiPath, 'profiles', `${options.profile}.json`), JSON.stringify(profile, null, 2) + '\n');
 ...
 */
function createSampleProfile(name) {
    return {
        version: '1.0',
        merge: 'deep',
        metadata: {
